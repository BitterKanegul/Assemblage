...
def build_hook(build_dir,
    ...
    res = cmd_with_output(f"{vcvarsall_loc} {arch} &&" +
                          f"cmake -S hermes -B build -G \"Visual Studio 17 2022\" -A {arch} " +
                          f"-DCMAKE_BUILD_TYPE={build_mode} &&" +
                          f"cmake --build build --config {build_mode}",
                          platform="windows", cwd=clone_dir, timelimit=600000)

arch = 'x64'

obj = CustomWindowsBuild(clone_dir='C:\\\\Hermes\\',
                         clone_flags=' --config core.autocrlf=false',
                         collect_dir="C:\\Binaries",
                         source_dir='C:\\\\Hermes\\',
                         build_dir=f'C:\\\\Hermes\\build\\',
                         project_git_url='https://github.com/facebook/hermes.git',
                         optimization='',
                         build_mode='Debug',
                         arch=arch,
                         tags=['v0.12.0', 'v0.11.0', 'v0.10.0', 'v0.9.0'],
